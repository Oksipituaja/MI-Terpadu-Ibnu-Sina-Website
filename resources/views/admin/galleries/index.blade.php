...
@section('content')
        ...
        <div>
            ...
            <p class="text-sm text-gray-500">
                @if ($search)
                    ...
                    &mdash; <span class="font-medium">{{ $galleries->total() }}</span> ditemukan
                @else
                    Total <span class="font-medium">{{ $galleries->total() }}</span> item galeri
                @endif
            </p>
        </div>
    ...
    {{-- SEARCH BAR --}}
    <form method="GET" action="{{ route('admin.galleries.index') }}" id="search-form" class="mb-5">
        <div class="flex items-center gap-3 max-w-sm">
            <div class="relative flex-1">
                ...
                <input type="text" name="search" id="search-input" value="{{ $search }}"
                    placeholder="Cari judul, kategori..." autocomplete="off" spellcheck="false"
                    ...
                    @forelse($galleries as $gallery)
                        <tr class="hover:bg-blue-50/40 transition-colors duration-150 group">
                            ...
                            <td class="px-6 py-3.5">
                                @if ($gallery->category)
                                    <span
                                        class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-purple-100 text-purple-700 whitespace-nowrap">
                                        {{ $gallery->category }}
                                    </span>
                                @else
                            ...
                            <td class="px-6 py-3.5">
                                <div class="flex items-center gap-2">
                                    ...
                                    <form action="{{ route('admin.galleries.destroy', $gallery) }}" method="POST"
                                        onsubmit="return confirmDelete('{{ addslashes($gallery->title) }}')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                            class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold
                            ...
                            <td colspan="4" class="px-6 py-20 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    <div class="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center">
                                        <i
                                            class="fas {{ $search ? 'fa-magnifying-glass' : 'fa-images' }} text-2xl text-gray-400"></i>
                                    </div>
                                    <div class="space-y-1">
                                        ...
                                        <p class="text-xs text-gray-400">
                                            {{ $search ? 'Coba kata kunci lain atau hapus pencarian' : 'Mulai tambahkan foto sekarang' }}
                                        </p>
                                    </div>
                                    @if ($search)
                                        <a href="{{ route('admin.galleries.index') }}"
                                            ...
                                            <i class="fas fa-arrow-left text-xs"></i> Lihat semua galeri
                                        </a>
                                    @else
                                        <a href="{{ route('admin.galleries.create') }}"
                                            class="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold
                                                  text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors">
                                            <i class="fas fa-plus text-xs"></i> Tambah Foto Pertama
                                        </a>
                                    @endif
                                </div>
                            </td>
...
@push('scripts')
    <script>
        ...
        function confirmDelete(name) {
            return confirm('Hapus foto "' + name + '"?\n\nData yang dihapus tidak dapat dikembalikan.');
        }
    </script>
@endpush
